...
注意，`ulem`包会默认将`\emph`命令的效果替换为下划线，可以在引入时规避。
...
加载单色emoji字体后，用`\char`或`\symbol`输入代码使用。emoji的unicode代码可以在[emoji字典](https://www.emojiall.com/)上查询。

典型的单色emoji字体如[Symbola](https://www.webfontfree.com/cn/download/Symbola)。

```TeX
% 代码前一个英文双引号为十六进制的标志
\fontspec{Symbola}{\symbol{"十六进制代码}}
\fontspec{Symbola}{\char{"十六进制代码}}
```
...
如果说半个字宽你还觉得影响不大的话，请注意，一段长文字必然要换行，而大部分标点不可以出现在行首，但是又要占一格。自然换行处是标点的话，*Typst*会从上一行扣一个字塞进下一行，使得前一行末尾出现整个字的空缺。

结果就是：随便粘几百字有对话的小说进*Typst*，右缘就稀烂了。如果文本是多语言夹杂的，这个问题还会更进一步放大。

#### 格式转换路径空缺

我写文一般用markdown，当下最简单的富文本格式。而就这么轻量级的语言，目前也没有md2typ的自动转化工具。注释、图片、表格这堆东西，要我自己动手扣格式，我是真的抠不动。
...
$\TeX$虽然长得很像英语，但其实是希腊词根*τεχ*的大写，具有technology和art双义，读作“泰-赫”。

$\LaTeX$的*La*则是*Lamport*的缩写。这个缩写和*Lamport*的读音没啥关系，多半只是为了继承$\TeX$反直觉的读法，所以名字故意与乳胶（latex）长得很像。它读作“拉-泰-赫”。

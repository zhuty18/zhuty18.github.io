...
不过，如果想插入较多较大的图片的话，*Overleaf*无能为力，还是需要自己安装*TeXLive*[^1]，在本地进行工作。
...
|标点|中文写法|外语写法|
|:--:|:--:|:--|
|省略号|`……`|`\ldots\ldots`，其后可能有`space`（一个空格）或`{}`（一组大括号）|
|左引号|`“`|``` `` ```（两个反引号）|
|右引号|`”`|`''`（两个单引号）|
...
加载单色emoji字体后，用`\char`或`\symbol`输入编码使用。emoji的unicode编码可以在[emoji字典](https://www.emojiall.com/)上查询。
